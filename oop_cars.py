...
class Table:
    def __init__(self):
        self.columns = ["Марка", "Модель", "Год выпуска", "Гос.номер", "Цвет", "Пробег", "ДТП"]
        self.table = []

    def display_table(self):
        if self.table:
            table_with_indices = [{"#": i, **row} for i, row in enumerate(self.table, start=1)]
            print(tabulate(table_with_indices, headers="keys", tablefmt="rounded_grid"))
        else:
            print("Таблица пуста")
    ...
    def save_table_without_print(self, file_name):
        if not self.table:
...
class Features(Table):
    ...
    def add_row(self):
        ...
        for column in self.columns:
            if column == "Марка":
                ...
                while any(row["Гос.номер"] == number for row in self.table):
                    print("Автомобиль с таким Гос.номером уже существует в таблице. Пожалуйста, введите другой номер.")
                    number = input("Введите Гос.номер: ").upper()
                new_row[column] = number
            elif column == "Пробег":
                while True:
                    probeg = input("Введите Пробег: ").upper()
                    if probeg:
                        new_row[column] = probeg
                        break
            elif column == "ДТП":
                while True:
                    DTP = input("Было ли ДТП: ").upper()
                    if DTP:
                        new_row[column] = DTP
                        break
            else:
        ...
        self.save_table_without_print("cars.txt")
    ...
    def search(self):
        ...
        if not found:
            ...
            print(tabulate(search_results, headers="keys", tablefmt="rounded_grid"))
...
while True:
    ...
    if choice == "1":
        ...
        features.save_table_without_print(file_name)
    elif choice == "7":
